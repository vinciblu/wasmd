...
func TestInitializeStaking(t *testing.T) {
	...
	valAddr := addValidator(t, ctx, stakingKeeper, k.Faucet, sdk.NewInt64Coin("silk", 1234567))
	...
	deposit := sdk.NewCoins(sdk.NewInt64Coin("denom", 100000), sdk.NewInt64Coin("silk", 500000))
...
func initializeStaking(t *testing.T) initInfo {
	...
	valAddr := addValidator(t, ctx, stakingKeeper, k.Faucet, sdk.NewInt64Coin("silk", 1000000))
	ctx = nextBlock(ctx, stakingKeeper)
...
func TestBonding(t *testing.T) {
	...
	full := sdk.NewCoins(sdk.NewInt64Coin("silk", 160000))
	funds := sdk.NewCoins(sdk.NewInt64Coin("silk", 80000))
	...
	assertSupply(t, ctx, keeper, contractAddr, "0", sdk.NewInt64Coin("silk", 0))
	...
	assertClaims(t, ctx, keeper, contractAddr, bob, "0")
	assertSupply(t, ctx, keeper, contractAddr, "80000", sdk.NewInt64Coin("silk", 80000))
}
...
func TestUnbonding(t *testing.T) {
	...
	assertSupply(t, ctx, keeper, contractAddr, "53000", sdk.NewInt64Coin("silk", 53000))
}
...
func TestReinvest(t *testing.T) {
	...
	full := sdk.NewCoins(sdk.NewInt64Coin("silk", 400000))
	funds := sdk.NewCoins(sdk.NewInt64Coin("silk", 200000))
	bob := initInfo.faucet.NewFundedRandomAccount(ctx, full...)

	// we will silk 200k to a validator with 1M self-bond
	...
	assertClaims(t, ctx, keeper, contractAddr, bob, "0")
	assertSupply(t, ctx, keeper, contractAddr, "200000", sdk.NewInt64Coin("silk", 236000))
}
...
func TestQueryStakingInfo(t *testing.T) {
	...
	assert.Equal(t, "silk", bondedRes.Denom)
	...
	// note this is not bob (who silkd to the contract), but the contract itself
	...
	require.Equal(t, wasmvmtypes.NewCoin(200000, "silk"), delInfo2.CanRedelegate)
	require.Len(t, delInfo2.AccumulatedRewards, 1)
	// see bonding above to see how we calculate 36000 (240000 / 6 - 10% commission)
	require.Equal(t, wasmvmtypes.NewCoin(36000, "silk"), delInfo2.AccumulatedRewards[0])
...
func TestQueryStakingPlugin(t *testing.T) {
	...
	require.Equal(t, wasmvmtypes.NewCoin(200000, "silk"), delInfo.CanRedelegate)
	require.Len(t, delInfo.AccumulatedRewards, 1)
	// see bonding above to see how we calculate 36000 (240000 / 6 - 10% commission)
	require.Equal(t, wasmvmtypes.NewCoin(36000, "silk"), delInfo.AccumulatedRewards[0])
...
func setValidatorRewards(ctx sdk.Context, stakingKeeper stakingkeeper.Keeper, distKeeper distributionkeeper.Keeper, valAddr sdk.ValAddress, reward string) {
	...
	payout := sdk.DecCoins{{Denom: "silk", Amount: amount}}
