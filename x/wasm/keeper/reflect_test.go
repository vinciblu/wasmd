...
func TestReflectTotalSupplyQuery(t *testing.T) {
	...
	currentSilkSupply := keepers.BankKeeper.GetSupply(ctx, "silk")
	require.NotEmpty(t, currentSilkSupply.Amount) // ensure we have real data
	specs := map[string]struct {
		...
		"known denom": {
			denom:     "silk",
			expAmount: ConvertSdkCoinToWasmCoin(currentSilkSupply),
		},
		"unknown denom": {
			denom:     "unknown",
